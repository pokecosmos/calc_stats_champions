...
function ncalc(num){
	...
	n += parseInt(document.nForm.elements[sn[num]].value) + 15;
...
function dcalc(num){
	...
	n = n - parseInt(document.nForm.elements[sn[num]].value)  - parseInt(15);
...
function ncalc2(num, dnum){
	...
	n = parseInt(dnum) + parseInt(document.nForm.elements[sn[num]].value) + 15;
...
function taikyu_hyoji_2(){//努力値オーバー排除、性格テキトー・ヌケニン排除
	...
	j=0; up=0; dw=0;
	for(i=0; i<6; i++){
		if((!document.nForm.elements[nn[i]].value)||(document.nForm.elements[dn[i]].value > 32)){
			j++;
		}
	}
...
function Keydown6() {
    ...
    for(i=0; i<6;i++){
    	if (document.activeElement === targetElement[nn[i]]) {
    		...
    		dchange(i,0);ncalc(i);dsum();return;
    	}else if (document.activeElement === targetElement[dnup[i]]) {
    		dchange(i,0);ncalc(i);dsum();return;
    	}else if (document.activeElement === targetElement[dndw[i]]) {
    		dchange(i,0);ncalc(i);dsum();return;
